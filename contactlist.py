def open_csv(csvfile:str) -> list:
    # csv_content, keys run this command to pull the csvfile you would like to work on return
...
def adding_user(keys:list,attributes:tuple):
    # takes in a tuple and converts it to a dict entry in csv_content
...
def find_user(keys:list,csv_content:list,name:str) -> dict:
    # returns the entry of a user by the first key value
...
csv_content,keys = open_csv('data.csv')
adding_user(keys,('alyssa','apples','orange'))
print(find_user(keys,csv_content,'matthew'))
update_attribute(keys,csv_content,'sam','favorite fruit','appercotts')
delete_record(keys,csv_content,'jacob')
update_file('data.csv',keys,csv_content)
